...
There are two solutions:
	...
	Another one is also using dictionary.
'''
"""
...
assert are_anagrams_dict('table', 'bleat') == True
assert are_anagrams_dict('table', 'bleate') == False
assert are_anagrams_dict('honey', 'eyhon') == True
assert are_anagrams_dict('area', 'are') == False
assert are_anagrams_dict('', '') == True

def time_test():
	"""a time test to evaluate the performance two methods
	results say anagrams_dict takes almost twice as much time :( """
	loops = 10000
	A = ""
	B = ""
	for x in range(1000):
	...
	s = time.clock()
	for i in range (loops):
		are_anagrams_dict(A, A_shuff)
	e = time.clock()
	print("time by are_anagrams_dict\t", e -s)
